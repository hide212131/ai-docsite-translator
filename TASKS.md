...
- [x] LangChain4j/Gemini をラップする翻訳インターフェースを定義し、dry-run／モック実装を用意する。
- [x] 行数・段落構成を維持する整形ヘルパーを実装する。
- [x] 新規差分のみを翻訳し既存翻訳にマージするルールを実装しテストする。
**完了条件チェックリスト**
- [x] 本番／dry-run／モックの切替が CLI オプションで制御できる。
- [x] 翻訳後のテキストが原文と行数一致するテストが通る。
- [x] 差分追加分のみが翻訳される統合テストが通る。
