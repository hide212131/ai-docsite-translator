...
- [ ] マージコンフリクトを自動解消する `ConflictDetector` / `ConflictResolutionPlanner` を実装し、既存訳を保持しつつ差分箇所のみ再翻訳するフローを整備する。
- [ ] upstream で新規に追加された行を検知し、既存訳がない領域を翻訳セグメントとして追加するロジックを `DiffAnalyzer` / `TranslationTaskPlanner` に実装する。
**完了条件チェックリスト**
