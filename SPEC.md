...
| `LLM_MAX_RETRY_ATTEMPTS` | レートリミット時の最大リトライ回数 | 任意 | 6 |
| `LLM_INITIAL_BACKOFF_SECONDS` | リトライ初回待機秒数（指数バックオフの基準値） | 任意 | 2 |
| `LLM_MAX_BACKOFF_SECONDS` | リトライ待機の最大秒数 | 任意 | 60 |
| `LLM_RETRY_JITTER_FACTOR` | リトライ待機時間のジッター係数（0.0～1.0、±変動率） | 任意 | 0.3 |

## 12. エラーハンドリング
- Git 操作失敗: リトライ後に致命的エラーとして終了。
- 翻訳 API エラー: エラー内容を記録し、対象ファイルはスキップして処理継続、最後に失敗一覧を出力。
- レートリミット対策（HTTP 429 / RESOURCE_EXHAUSTED）:
  - 指数バックオフ + ジッター方式のリトライを実装
  - 待機時間は `initialBackoff × 2^(試行回数-1)` で計算し、`maxBackoff` で上限制御
  - ジッター係数（デフォルト 0.3）でランダムな変動（±30%）を加え、複数リクエストの衝突を回避
  - API プロバイダから retry-after ヘッダが得られる場合は優先的に使用
  - 最大リトライ回数（デフォルト 6 回）を超えた場合は、該当ファイルを失敗リストに記録して処理継続
  - ログには試行回数、計算された待機時間、最終結果（成功/失敗）を記録
