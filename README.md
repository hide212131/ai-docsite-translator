...
### 判定基準
- **小規模コンテンツ（200文字未満）**: 変更文字数が 15 文字以下であれば軽微な変更と判定
- **大規模コンテンツ（200文字以上）**: 変更文字数がコンテンツ全体の 10% 以下であれば軽微な変更と判定
- 判定にはレーベンシュタイン距離（編集距離）を使用して、変更の実質的な大きさを評価します
...
### スキップ時の挙動
- 翻訳処理がスキップされ、LLM API への呼び出しが行われません
- ログに `Skipping translation for {ファイル名} due to minor changes only (typo/spelling fix)` が記録されます
- upstream の変更（英語の修正）は origin 側に取り込まれますが、既存の翻訳文は保持されます

この機能により、typo 修正のような軽微な変更によって翻訳が不必要に更新されることを防ぎ、翻訳コストを削減できます。
